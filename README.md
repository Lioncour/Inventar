...
3. Add repository secrets:
   ...
   - `REMOVE_BG_API_KEY`
   - `IMAGGA_API_KEY` and `IMAGGA_API_SECRET`
...
- Filters only match items that actually have those tags. Most auto-tags come from Imagga after processing.
- The workflow no longer runs on every `items.json` push, so processing commits cannot retrigger themselves.
- `npm ci` is not used in Actions because a stale lockfile previously broke installs. Runtime packages are installed into `.ci-node` for the job only.
